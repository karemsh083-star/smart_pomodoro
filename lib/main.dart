...
class _PomodoroTimerScreenState extends State<PomodoroTimerScreen> {
  ...
  @override void initState() { super.initState(); _loadPersistentData(); }

  // دالة جلب الساعات والدقائق المخزنة بداخل الذاكرة الأبدية للهاتف عند التشغيل
  void _loadPersistentData() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    setState(() { totalFocusSecondsSaved = prefs.getInt('totalFocusSeconds') ?? 0; });
  }

  // دالة حفظ الساعات والدقائق الجديدة تلقائياً ثانية بثانية داخل ذاكرة الهاتف لكي لا تصفر
  void _savePersistentData() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setInt('totalFocusSeconds', totalFocusSecondsSaved);
  }
  ...
  void _openTimerSettingsSheet() {
    showModalBottomSheet(context: context, backgroundColor: const Color(0xff111827), shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(24))), builder: (BuildContext context) {
        return StatefulBuilder(builder: (BuildContext context, StateSetter setSheetState) {
            return Container(padding: const EdgeInsets.all(24.0), height: 320, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Expanded(child: Row(children: [
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
                              ...
                              DropdownButton<String>(value: _selectedIntervalType, dropdownColor: const Color(0xff1F2937), isExpanded: true, underline: Container(), items: <String>["25/5", "50/10", "75/15", "100/20"].map((String val) { return DropdownMenuItem<String>(value: val, child: Text('Mode $val', style: const TextStyle(fontSize: 13))); }).toList(),
                              ...
                              const SizedBox(height: 6),
                              DropdownButton<int>(value: _sessionsBeforeLongBreak, dropdownColor: const Color(0xff1F2937), isExpanded: true, underline: Container(), items: const [DropdownMenuItem<int>(value: 2, child: Text('After 2 Sessions', style: TextStyle(fontSize: 13))), DropdownMenuItem<int>(value: 3, child: Text('After 3 Sessions', style: TextStyle(fontSize: 13))), DropdownMenuItem<int>(value: 4, child: Text('After 4 Sessions', style: TextStyle(fontSize: 13))), DropdownMenuItem<int>(value: 5, child: Text('After 5 Sessions', style: TextStyle(fontSize: 13)))], onChanged: (val) { if (val != null) { setState(() => _sessionsBeforeLongBreak = val); setSheetState(() => _sessionsBeforeLongBreak = val); } },),
                            ],
                          ),
                        ),
                        const VerticalDivider(color: Colors.white10, width: 40),
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Long Break Duration', style: TextStyle(fontSize: 13, color: Colors.grey, fontWeight: FontWeight.bold)), Expanded(child: ListWheelScrollView.useDelegate(itemExtent: 35, perspective: 0.005, diameterRatio: 1.2, physics: const FixedExtentScrollPhysics(), onSelectedItemChanged: (index) { int newMins = index + 5; setState(() => _longBreakMinutes = newMins); setSheetState(() => _longBreakMinutes = newMins); }, childDelegate: ListWheelChildBuilderDelegate(childCount: 26, builder: (context, index) { int currentMinuteItem = index + 5; bool isSelected = currentMinuteItem == _longBreakMinutes; return Center(child: Text('$currentMinuteItem mins', style: TextStyle(fontSize: isSelected ? 16 : 14, fontWeight: isSelected ? FontWeight.bold : FontWeight.normal, color: isSelected ? const Color(0xff38BDF8) : Colors.white30))); }))),])),
                      ],
                    ),
                  ),
                ],
  ...
  void _startTimer() { if (_isRunning) return; setState(() { _isRunning = true; }); _timer = Timer.periodic(const Duration(seconds: 1), (timer) { setState(() { if (_totalSeconds > 0) { _totalSeconds--; if (_currentMode == "pomodoro") { totalFocusSecondsSaved++; _savePersistentData(); } } else { _timer?.cancel(); _isRunning = false; _showBreakDialog(); } }); }); }
  ...
  @override Widget build(BuildContext context) {
    ...
    int displayHours = totalFocusSecondsSaved ~/ 3600; int displayMins = (totalFocusSecondsSaved % 3600) ~/ 60;
    return Scaffold(
      body: Container(
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                Text("Total Focus Time: $displayHours hours $displayMins mins", style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Color(0xff38BDF8), letterSpacing: 0.5)),
                const SizedBox(height: 6), Container(height: 235, alignment: Alignment.center, child: _isFlipClockMode ? _buildFullFlipClockStyle(minStr, secStr) : _buildProgressCircleStyle("$minStr:$secStr", progressPercentage, circleColor)),
